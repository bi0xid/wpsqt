			...
			if ( $this->_step == 1 ){
				...
				foreach ( $fields as $key => $field ){
					...
					if ( !isset($_POST["Custom_".$fieldName]) || empty($_POST["Custom_".$fieldName]) ){
						...
						if($fields[$key]['validation'] == 'DNI'){
							$dni = $fields[$key]['value'];
							
							$is_valid_and_unique_dni = 1;
							$is_valid_dni = valida_dni($dni);
							if($is_valid_dni){
								
								$is_valid_and_unique_dni = valida_es_unico_dni($dni);	
							}
							
							if($is_valid_and_unique_dni && $is_valid_dni){
								$field['value'] = $_POST["Custom_".$fieldName];
								$_SESSION['wpsqt'][$quizName]['person'][ strtolower($field['name']) ] = $_POST["Custom_".$fieldName];
							}else{
								if(!$is_valid_and_unique_dni){ $errors[] = 'Ja has votat amb aquest DNI: ' .$dni; }
								if(!$is_valid_dni){ $errors[] = 'Escriu un '.$field['name'].' correcte'; }
								
								$all_votes = $wpdb->get_results( "SELECT id,person FROM $wpdb->wpsqt_all_results WHERE item_id = 1" );
								//$errors[] = 'ALL VOTES:'.print_r($all_votes);
							}
...
/**
	 * DNI validation.
	 *
	 * @since Mecus Consultans modification 01/08/2012
*/

function valida_dni($dni) // retorna false(0) si hay error o el DNI validado y con letra si no hay error
{
   $str = trim($dni);
   $str = str_replace("-","",$str);
...
/**
	 ...
	 *
	 * @since Mecus Consultans modification 28/09/2012
	 * @return false(0) si ya había alguien votado con ese DNI, devuelve el DNI de la votación actual en caso de ser la primera vez
*/
function valida_es_unico_dni($dni){
	
	global $wpdb;
	
	$item_id = $_SESSION['wpsqt']['item_id'];
	
	$all_votes = $wpdb->get_results( "SELECT * FROM wp_wpsqt_all_results WHERE item_id = $item_id" );
	
	foreach($all_votes as $all_vote){
		$person = unserialize( $all_vote->person );
		if($person['dni'] == $dni){
			return 0;
		}
	}
